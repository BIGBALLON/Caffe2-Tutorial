 ...
 * by using pre-trained ImageNet ResNet-50 model.
 *
...
std::string labelToName(int index);
...
void run(){
    ...
    float imgArray[3 * IMAGE_SIZE * IMAGE_SIZE];
    loadImage(FLAGS_file, imgArray);

    // define a Tensor which is used to stone input data
    TensorCPU input;
    input.Resize(std::vector<TIndex>({1, 3, IMAGE_SIZE, IMAGE_SIZE}));
