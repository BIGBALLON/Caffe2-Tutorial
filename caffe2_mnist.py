...
def Add_LeNet_Model(model, data, device_opts,is_test=False):
    with core.DeviceScope(device_opts):
        conv1 = brew.conv(
            model, 
            data, 
...
def AddTrainingOperators(model, softmax, label, device_opts):
    with core.DeviceScope(device_opts):
        ...
        # Feel free to change the parameters[e.g. gamma, stepsize, etc.]
        # Or use other optimizers [e.g. adam ,adagrad]
        # build_adam(...) build_adagrad(...)
        optimizer.build_sgd(
            model,
...
def DoTrain():
    ...
    softmax = Add_LeNet_Model(train_model, data, DEVICE_OPTS)
    ...
    softmax = Add_LeNet_Model(test_model, data, DEVICE_OPTS)
    ...
    Add_LeNet_Model(deploy_model, "data", DEVICE_OPTS, True)
...
def DoImageTest():
    ...
    import cv2
    img = cv2.imread( './test_img/9.jpg' )                      # Load test image
...
if __name__ == '__main__':
    ...
    # We suggest you to use GPU since the speed of CPU is very slow
